...
public class MultiCorePayV2Client : PayV2ClientBase {
	...
	/// <summary>
	///     Create a new Multicore PAY client. Call <see cref="Initialize"/> first.
	/// </summary>
	...
	public void Initialize() {
		if (!IsInitialized) {
			...
			if (response?.TguList == null) {
				throw new PayNlException("Unable to retrieve available cores from PAY");
			}
	...
	public Task<CreateTransactionResponse?> CreateTransactionMultiCore(CreateTransactionRequest body) => _multiCoreRequester.ExecuteWithAutomaticCoreSwitching(() => _payV2Client.CreateTransaction(body, AvailableCores[ActiveEndpointIndex]));
	...
	public Task<GetTransactionResponse?> GetTransactionMultiCore(string transactionId) => _multiCoreRequester.ExecuteWithAutomaticCoreSwitching(() => _payV2Client.GetTransactionInfo(transactionId, AvailableCores[ActiveEndpointIndex]));
	...
	public Task<CancelTransactionResponse?> CancelTransactionMultiCore(string transactionId) => _multiCoreRequester.ExecuteWithAutomaticCoreSwitching(() => _payV2Client.CancelTransaction(transactionId, AvailableCores[ActiveEndpointIndex]));
}
